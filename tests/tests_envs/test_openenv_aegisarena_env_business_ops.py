...
def test_aegisarena_business_ops_reset_forced() -> None:
    ...
    assert payload["info"]["mission_type"] == "business_ops"
    assert payload["observation"]["mission_type"] == "business_ops"
    assert payload["state"]["mission_type"] == "business_ops"
    assert payload["state"]["hidden_truth"]["expected_answer"] in {
        "escalate_ops",
        "priority_queue",
        "standard_queue",
    }
...
def test_aegisarena_business_ops_query_tool_ticket_lookup() -> None:
    ...
    assert payload["info"]["tool_name"] == "ticket_lookup"
    assert payload["state"]["mission_type"] == "business_ops"
...
def test_aegisarena_business_ops_submit_final_success() -> None:
    reset_response = client.post(
        "/reset",
        json={
            "seed": 12345,

...
def test_aegisarena_finance_ops_reset_forced() -> None:
    ...
    assert payload["info"]["mission_type"] == "finance_ops"
    assert payload["observation"]["mission_type"] == "finance_ops"
    assert payload["state"]["mission_type"] == "finance_ops"
...
def test_aegisarena_finance_ops_query_tool_table_lookup() -> None:
    ...
    assert payload["info"]["tool_name"] == "table_lookup"
    assert payload["state"]["mission_type"] == "finance_ops"
...
def test_aegisarena_finance_ops_submit_final_success() -> None:
    reset_response = client.post(
        "/reset",
        json={
            "seed": 12345,
